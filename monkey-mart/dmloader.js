var FileLoader = {
    ...
    request: function(url, method, responseType, currentAttempt = 0) {
        if (!method || !responseType) {
            throw "No method or responseType specified";
        }
        ...
        return {
            send: function() {
                ...
                xhr.onprogress = e => this.onprogress && this.onprogress(xhr, e);
                xhr.onerror = e => {
                    if (currentAttempt === FileLoader.options.retryCount) {
                        this.onerror && this.onerror(xhr, e);
                    } else {
                        currentAttempt++;
                        setTimeout(this.send.bind(this), FileLoader.options.retryInterval);
                    }
                };
                xhr.onload = e => this.onload && this.onload(xhr, e);
    ...
    size: function(url, callback) {
        ...
        request.onerror = () => callback(undefined);
    ...
    load: function(url, responseType, estimatedSize, onprogress, onerror, onload) {
        ...
        request.onprogress = function(xhr, e) {
            if (e.lengthComputable) {
                ...
                onprogress(e.loaded, size || e.loaded);
            }
        };
        ...
        request.onload = function(xhr) {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    ...
                    onerror(`Error loading '${url}' (${xhr.statusText})`);
                }
            }
        };
        request.send();
