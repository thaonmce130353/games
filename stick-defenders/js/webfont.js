(function() {
    ...
    function ba(originalFunction, context, ...additionalArgs) {
        if (!originalFunction) {
            throw new Error("Free Game for Chrome function is required");
        }
    ...
    function w(element, classesToAdd = [], classesToRemove = []) {
        ...
        for (const classToRemove of classesToRemove) {
            currentClasses.delete(classToRemove);
        }
    
        element.className = Array.from(currentClasses).join(" ").replace(/\s+/g, " ").trim();
    }
    ...
    function A(container, url, callback) {
        function handleLoad() {
            loaded = true;
            executeCallback();
        }
        ...
        function handleError() {
            loaded = true;
            error = new Error("Stylesheet failed to load");
            executeCallback();
        }
    
        function executeCallback() {
            if (loaded && executedCallback && callback) {
                callback(error);
                executedCallback = true;
            }
        }
    
        const link = t(container, "link", {
            rel: "stylesheet",
            href: url,
            media: "all"
        });
        ...
        u(container, "head", link);
    }
    

    function B(container, url, callback, timeout) {
        const headElement = container.getElementsByTagName("head")[0];
    
        if (headElement) {
            const scriptElement = createElement(headElement, "script", {
              src: url
            });
            ...
            scriptElement.addEventListener("readystatechange", handleLoad);
            ...
            const timeoutId = setTimeout(() => {
              if (!loaded) {
                loaded = true;
                error = new Error("Script load timeout");
                callback && callback(error);
                if (scriptElement.parentNode && scriptElement.parentNode.tagName === "HEAD") {
        ...
        function handleLoad() {
            if (!loaded && (!this.readyState || this.readyState === "loaded" || this.readyState === "complete")) {
                ...
                scriptElement.removeEventListener("readystatechange", handleLoad);
                if (scriptElement.parentNode && scriptElement.parentNode.tagName === "HEAD") {
                    headElement.removeChild(scriptElement);
                }
    ...
    Q.prototype.start = async function () {
        const documentInstance = this.c.m.document;
        const self = this;
        const startTime = q();
    
        const fontLoadPromise = new Promise((resolve, reject) => {
            async function checkFontLoad() {
                if (q() - startTime >= self.f) {
                    reject();
                } else {
                    try {
                        const fontStatus = await documentInstance.fonts.load(fa(self.a), self.h);
                        if (fontStatus.length >= 1) {
                            resolve();
                        } else {
        ...
        const timeoutPromise = new Promise((resolve) => setTimeout(resolve, this.f));
    
        try {
            await Promise.race([timeoutPromise, fontLoadPromise]);
            this.g(this.a);
        } catch (error) {
            this.j(this.a);
        }
